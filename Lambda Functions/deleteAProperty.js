...
exports.handler = async(event, context) => {
    ...
    if (sharedAccountInfo === null) {
        ...
        response = {
            ...
            body: JSON.stringify({
                statusCode: 400,
                message: "Couldn't find the account you're trying to remove a property , please refresh and try again",
            }),
        };
        return response;
    }
    ...
    // Get all the shared properties for the shared device
    var propertyParams = {
    ...
    const propertyResults = await documentClient
        .query(propertyParams, function(err, data) {
            if (err) {
                console.error(
                    "Unable to query shared_device_properties. Error:",
                    JSON.stringify(err, null, 2)
    ...
    if (property == null) {
        console.log("Couldn't find the property in the request");
        response = {
            statusCode: 400,
            body: JSON.stringify({
                statusCode: 400,
                message: "Error while searching for the specified property, please refresh and try again",
            }),
        };
        return response;
    }
    
    // Delete the property from the shared_property table
    const deleteParams = {
        TableName: "Shared_Property",
        Key: {
            "shared_property_id": property.shared_property_id
        }
    };
